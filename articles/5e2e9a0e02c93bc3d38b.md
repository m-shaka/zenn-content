...
詳しくは後述しますが、wasmとその実行環境（ホスト環境とも呼ばれます）は関数をimport/exportできるので、OSの機能もそれを通して渡してあげればwasmからも使うことができます。ただ、各実装が好き勝手なインターフェースでやってしまうと、特定のホスト環境に依存したwasmプログラムを作らねばならず可搬性が下がります。そこで提案されたのがWASI（WebAssembly System Interface）です。WASIはwasmプログラムからOSの機能を使うための決まりごとのようなものです。WASIに従ってwasmを作れば、WASIに対応する全てのホスト環境で実行することができるのです。<!-- TODO: ソフトウェアデザインの紹介-->。上で紹介したWasmtimeやWasmerはWASIランタイムの1つです。
...
wasmとWASIがメインなのでそこが一番厚いです。Brainfuckの話はほぼおまけです。そして、以下の内容は**扱いません**。
- ブラウザでの実行方法
...
これからwatコードを見つつ説明していきますが、それに先立って簡易的な実行環境を紹介しておきたいと思います。次のNode.jsスクリプトは、ファイルから読んだwatのコードをwabtでバイナリに変換してからコンパイルするものです。
...
`npm i wabt`で依存を入れてから`node run_wat.js path/to/wat/file`を実行して下さい。実はこのスプリクトでは何も起こらないのですが、読み込んだwatファイルに不具合があればコンパイルエラーで教えてくれます。`importObject`も空ですが後で説明します。
...
モジュール内には様々なコンポーネントを配置できますが、今回必要なのは`func`, `global`, `memory`, `export`, `import`の5つです。実際の処理内容は`func`の中身に記述していくため一番重要なのは`func`です。まずは`func`の宣言方法と処理の記述方法について見てみましょう。なお、以下のコード例では断りなく`module`宣言を省くことがあります。予めご了承下さい。
...
## exportとimport
wasmコードはなんらかの実行環境（ホスト環境とも呼ばれます）から呼び出す必要があります。ホスト環境から参照したいwasmのコンポーネントは、`export`で明示的に宣言してあげる必要があります。
...
空だった`importObject`を修正しました。これを`WebAssembly.instantiate`に渡せば`console.log`がwasmから参照できます。`add.wat`も変更しましょう。
